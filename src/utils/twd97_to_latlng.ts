//source code. https://timshyue.com/js-twd97-轉-wgs84/
...
/**
 * @param {number} twd97x The value of twd97 in x direction.
 * @param {number} twd97y The value of twd97 in y direction.
 *
 * @returns {LatLng} The latitude and longitude of the location.
 */
export const twd97_to_latlng = (twd97x: number, twd97y: number): LatLng => {
	let pow: Function = Math.pow,
	...
	// 	Q4 =
	// 		((61 + 90 * T1 + 298 * C1 + 45 * pow(T1, 2) - 3 * pow(C1, 2) - 252 * e2) * pow(D, 6)) / 720.0,
	// 	$lat = fp - Q1 * (Q2 - Q3 + Q4),
	// 	Q5 = D,
	...
	var $mu =
		$M / ($a * (1.0 - pow($e, 2) / 4.0 - (3 * pow($e, 4)) / 64.0 - (5 * pow($e, 6)) / 256.0))
	...
	var $Q4 =
		((61 + 90 * $T1 + 298 * $C1 + 45 * pow($T1, 2) - 3 * pow($C1, 2) - 252 * $e2) * pow($D, 6)) /
		720.0
	...
	var $Q7 =
		((5 - 2 * $C1 + 28 * $T1 - 3 * pow($C1, 2) + 8 * $e2 + 24 * pow($T1, 2)) * pow($D, 5)) / 120.0
